...
function ProfileScreen({profile, onSave, onBack}){
  ...
  function saveUsername(){
    ...
    // Update auth db (works for both email and Google accounts)
    const key=Object.keys(db).find(k=>db[k].username===oldName);
    if(key){ db[key]={...db[key],username:u}; saveDB(db); }

    // Update pkmn_profiles list
    const profiles=JSON.parse(localStorage.getItem("pkmn_profiles")||"[]");
    localStorage.setItem("pkmn_profiles",JSON.stringify(profiles.map(p=>p.name===oldName?{...p,name:u}:p)));

    // Update all per-profile localStorage keys to use new name
    const keysToRename=[
      `pkmn_unlocked_${oldName}`,`pkmn_inv_${oldName}`,`pkmn_fainted_${oldName}`,
      `pkmn_team_${oldName}`,`pkmn_levels_${oldName}`,`pkmn_starter_${oldName}`,
      `pkmn_exp_${oldName}`,`pkmn_history_${oldName}`,`pkmn_streak_${oldName}`,
    ];
    keysToRename.forEach(k=>{
      const val=localStorage.getItem(k);
      if(val!==null){
        localStorage.setItem(k.replace(oldName,u),val);
        localStorage.removeItem(k);
      }
    });

    // Update active session so refresh restores correct name
  ...
  function handlePlayerFaint(){
    ...
    setTimeout(async()=>{
      ...
      setProfile(p=>{const np={...p,losses:(p.losses||0)+1};updateProfile(np);
        supaUpdateStats(np.name,np.wins||0,np.losses,np.coins||0); // track all users
        return np;});
  ...
  function handleEnemyFaint(){
    ...
    setTimeout(async()=>{
      ...
      setProfile(p=>{ const np={...p,wins:(p.wins||0)+1}; updateProfile(np);
        supaUpdateStats(np.name,np.wins,np.losses||0,np.coins||0); // track all users
        return np; });
